    ...
    children: [
        {
            ...
            userProfile: basant,
            children: [
                { id: 3, name: "Arijit Saha", region: "EAST" },
                { id: 4, name: "Arghyadeep Ghosh", region: "EAST", },
...
const StatusDot: React.FC<{ status?: OrgNodeType["status"] }> = ({
    ...
    return (
        <span
            className={`h-2 w-2 rounded-full ${status === "onTrack" ? "bg-green-500" : "bg-red-500"
                }`}
        />
    );
};
...
const NewDashboard: React.FC = () => {
    ...
    const [selectedNodeData, setSelectedNodeData] = useState<IDSRData[]>(dsrdata.filter(item => item.id === 1));
    useEffect(() => {
            ...
            <section className="w-full lg:flex">
                ...
                <div className="w-full lg:pl-2 lg:mt-0 mt-4">
                    <div className="h-full w-full bg-gradient-to-l from-[#daf0ff] to-[#F0F9FF] rounded-lg">
                        ...
                        <div className="">
                            <SalesDashboard active={active} selectedNodeData={selectedNodeData || [] as IDSRData[]} />
                        </div>
                    </div>
                </div>
            </section>
