...
namespace BERGER_ONE_PORTAL_API.Repository.Login
{
	public class LoginRepo:ILoginRepo
	{
		...
		public async Task<MSSQLResponse> GetUserApplicableMenuVr1(string UserId, string UserGroup, Int32 AppId)
		{
			MSSQLResponse? response = null;
			var sqlParams = new SqlParameter[3];
			...
			response = new MSSQLResponse()
			{
				Data = await _sqlHelper.FetchData(new ExecuteDataSetRequest()
				{
					CommandText = "[dbo].[LoginUserAccessForms_Get_Vr1]",
					CommandTimeout = Constant.Common.SQLCommandTimeOut,
					CommandType = CommandType.StoredProcedure,
					ConnectionProperties = _serviceContext.MSSQLConnectionModel,
					IsMultipleTables = false,
		...
		internal static List<UserApplicableMenuModel>? MapUserApplicableMenuVr1(DataTable? dt)
		{
			var data = new List<UserApplicableMenuModel>();
			if (dt != null && dt.Rows.Count > 0)
			{
				data = dt.AsEnumerable()?.Where(nav => Convert.ToInt32(nav.Field<object>("fmm_parent_id")) == 0)?.Select(nav => new UserApplicableMenuModel
				{
					...
					Children = dt.AsEnumerable().Where(x => Convert.ToString(x.Field<object>("fmm_parent_id")).StringEquals(Convert.ToString(nav.Field<object>("fmm_id")), StringComparison.OrdinalIgnoreCase))?.Select(r => new UserApplicableMenuModel
					{
						...
						fafa_icon = Convert.ToString(r.Field<object>("fmm_menu_icon")),
						form_seq = Convert.ToInt32(r.Field<object>("fmm_sequence")),
						Children = dt.AsEnumerable().Where(x => Convert.ToString(x.Field<object>("fmm_parent_id")).StringEquals(Convert.ToString(r.Field<object>("fmm_id")), StringComparison.OrdinalIgnoreCase))?.Select(r1 => new UserApplicableMenuModel
						{
							form_id = Convert.ToInt32(r1.Field<object>("fmm_id")),
							form_name = Convert.ToString(r1.Field<object>("fmm_name")),
							form_link = Convert.ToString(r1.Field<object>("fmm_link")),
							form_parent_id = Convert.ToInt32(r1.Field<object>("fmm_parent_id")),
							fafa_icon = Convert.ToString(r1.Field<object>("fmm_menu_icon")),
							form_seq = Convert.ToInt32(r1.Field<object>("fmm_sequence"))
						})?.ToList(),
					})?.ToList(),
				}).DistinctBy(r => r.form_id)?.ToList();
			}
