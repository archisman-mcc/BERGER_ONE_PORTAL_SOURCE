...
namespace BERGER_ONE_PORTAL_API.Logic
{
    public class LoginLogic : ILoginLogic
    {
		...
		public async Task<LoginResponseDto?> ValidateLoginVr1(LoginRequestDto? request)
		{
			LoginResponseDto response = new LoginResponseDto();
			...
			if (dbResponse != null)
			{
				...
				if (outputCode > 0 && dt != null && dt.Rows.Count > 0)
				{
					...
					if (usermapping != null)
					{
						var menuData = await _loginRepo.GetUserApplicableMenuVr1(Convert.ToString(dt.Rows[0]["user_id"]), Convert.ToString(dt.Rows[0]["usp_group_code"]), 15);

						if (usermapping != null && (menuData != null && menuData.Data.Rows.Count > 0)) usermapping.UserApplicableMenu = LoginRepo.MapUserApplicableMenuVr1(menuData.Data as DataTable);
		...
		public async Task<LoginResponseDto?> ValidateRefreshTokenV2(TokenRefreshDtoNew? tokenRefreshNewDto)
		{
			LoginResponseDto response = new LoginResponseDto();
