...
namespace BERGER_ONE_PORTAL_API.Logic
{
    public class CommonLogic : ICommonLogic
    {
        ...
        public async Task<UserAppsResponseDto?> GetAppList_Vrn1(UserApplicableMenuReqModel dto)
        {
            MSSQLResponse? dataResponse = await _commonRepo.GetAppListData_Vrn1(dto);
            return UserAdapter.MapAppResponse(dataResponse);
        }
        ...
        public async Task<GetRegionResponseDto?> GetProtectonApplicableTerr(GetProtectonApplicableTerrRequestDto? request, string userid)
        {
            GetRegionResponseDto response = new GetRegionResponseDto();
            var dbResponse = await _commonRepo.GetProtectonApplicableTerr(request, userid);
            if (dbResponse != null)
            {
                var ds = dbResponse.Data as DataSet;
                if (ds != null && ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
